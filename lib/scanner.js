...
export async function scan(onProgress) {
  ...
  // Scan promotions + updates + newsletters categories (no date limit = full history).
  // Separate pass for primary inbox covers newsletters outside categories (e.g. gaming stores).
  const query = 'category:promotions OR category:updates OR category:newsletters OR (in:inbox -in:chats)';
  ...
  console.log('[Scanner] message IDs fetched:', ids.length);
