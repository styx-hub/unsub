...
export async function scan(onProgress, lang = 'sk') {
  abortFlag = false;
  const s = sws(lang);
  ...
  onProgress({ pct: 2, label: s.scanLoading });

  const ids = await listMessageIds(query, maxResults, (fetched, total) => {
    const pct = Math.round((fetched / total) * 15) + 2;
    onProgress({ pct, label: s.scanIds(fetched, Math.min(total, maxResults)) });
  });

  if (abortFlag) return null;

  onProgress({ pct: 18, label: s.scanMessages(ids.length) });
  ...
  const metaTasks = ids.map(id => async () => {
    ...
    if (email.endsWith('@google.com') || email.endsWith('@googlemail.com')) { processed++; return; }
    ...
    if (listUnsub) {
      ...
      if (senderMap.has(email)) {
        ...
        ex.count++;
        if (msgDate > ex.lastMessageDate) ex.lastMessageDate = msgDate;
        ...
        senderMap.set(email, {
          email, displayName: name || email, count: 1, lastMessageDate: msgDate,
      ...
      if (candidatesForBodyScan.has(email)) {
        const c = candidatesForBodyScan.get(email);
        c.count++;
        if (msgDate > c.lastDate) c.lastDate = msgDate;
      } else {
        candidatesForBodyScan.set(email, { id, name: name || email, count: 1, lastDate: msgDate });
    ...
    onProgress({ pct, label: s.scanProgress(processed, ids.length) });
  });
  ...
  if (bodyEntries.length > 0) {
    onProgress({ pct: 74, label: s.scanBody(bodyEntries.length) });
    ...
    const bodyTasks = bodyEntries.map(([email, { id, name }]) => async () => {
      ...
      try {
        ...
        if (url) {
          const candidate = candidatesForBodyScan.get(email);
          senderMap.set(email, {
            email, displayName: name, count: candidate?.count || 1,
            lastMessageDate: candidate?.lastDate || 0,
      ...
      onProgress({ pct, label: s.scanBodyProg(bodyDone, bodyEntries.length) });
    });
  ...
  onProgress({ pct: 100, label: s.scanDone(senders.length) });
  return senders;
}
