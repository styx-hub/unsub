...
export async function scan(onProgress) {
  ...
  // No category filter — scan all recent mail and let List-Unsubscribe header be the filter.
  // newer_than:6m keeps it to recent newsletters and limits API calls.
  const query = 'newer_than:6m -in:chats -in:sent -in:drafts';
