...
async function apiFetch(url, options = {}, attempt = 0) {
  ...
  if (resp.status === 401) {
    if (attempt > 0) throw new Error('Auth failed after token invalidation');
    console.warn('[API] 401 — invalidating cached token and retrying');
...
// Fetch full message including body (for body-link unsubscribe detection).
export async function getMessageFull(messageId) {
  return apiFetch(`${BASE}/messages/${messageId}?format=full`);
}
