...
export async function getMessageMetadata(messageId) {
  const params = new URLSearchParams({
    format: 'metadata',
    metadataHeaders: ['From', 'List-Unsubscribe', 'List-Unsubscribe-Post'].join(','),
  });
...
// Archive messages by removing INBOX label (Phase 6).
export async function archiveMessages(messageIds) {
  return apiFetch(`${BASE}/messages/batchModify`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
