...
// Find unsubscribe URL in HTML body — looks for anchor text or href containing unsub keywords.
export function findBodyUnsubscribeUrl(html) {
  if (!html) return null;
  const doc = new DOMParser().parseFromString(html, 'text/html');
  const re = /unsubscribe|opt[\s-]?out|odhlás/i;

  for (const a of doc.querySelectorAll('a[href]')) {
    const href = a.getAttribute('href') || '';
    const text = a.textContent.trim();
    if ((re.test(text) || re.test(href)) && href.startsWith('http')) {
      return href;
    }
  }
  return null;
