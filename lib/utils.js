...
// Recursively find HTML body part in Gmail MIME payload.
...
// Find unsubscribe URL in HTML body using regex (DOMParser unavailable in SW).
// Handles both: <a href="...">Unsubscribe</a> AND "To unsubscribe, click <a href="...">here</a>"
export function findBodyUnsubscribeUrl(html) {
  if (!html) return null;
  const unsubRe = /unsubscribe|opt[\s-]?out|odhlás/i;

  // Pass 1: link text or href contains unsub keyword
  const anchorRe = /<a\s[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;
  let match;
  while ((match = anchorRe.exec(html)) !== null) {
    const href = match[1].trim();
    const text = match[2].replace(/<[^>]+>/g, '').trim();
    if (href.startsWith('http') && (unsubRe.test(href) || unsubRe.test(text))) {
      return href;
    }
  }

  // Pass 2: "unsubscribe" appears within 300 chars before a link (e.g. "click here")
  const contextRe = /unsubscribe[\s\S]{0,300}?<a\s[^>]*href=["']([^"']+)["']/gi;
  while ((match = contextRe.exec(html)) !== null) {
    const href = match[1].trim();
